...
function selectCustomer(cid, cname, cadd1, cadd2, ph, email, discount = 0, company = '') {
    ...
    if ($("#customer_address").length) {
        $('#customer_address').val(cadd1 + ', ' + cadd2);
    }
    if ($("#customer_address1").length) {
...
function selectEmployee(eid, ename, eadd1, eadd2, ph, email, discount = 0) {
    ...
    $('#employee_name').html('<strong>' + ename + '</strong>');
    $('#employee_name').val(ename);
    $('#employee_address1').html('<strong>' + eadd1 + '<br>' + eadd2 + '</strong>');
    $('#employee_phone').html('Phone: <strong>' + ph + '</strong><br>Email: <strong>' + email + '</strong>');
    $("#employee-box").val();
    $("#employee-box-result").hide();
    $(".sbox-result").hide();
    $("#employee").show();
}
...
if (userfileElement) {
  userfileElement.addEventListener("change", function(event) {
    var allowedExtensions = ["docx", "doc", "txt", "pdf", "xls", "png", "jpg", "gif"];
...
$("#submit-update").on("click", function (e) {
    e.preventDefault();
    $(this).hide();
   // var o_data = $("#update_form").serialize();
    var o_data = $("#update_form").find(':input').not('.no-serialize').serialize();
    var action_url = $('#update-url').val();
    var table_reload = $('#table_reload').val();
    //alert(table_reload)
    addObject(o_data, action_url, table_reload);
    console.log(o_data);
    setTimeout(function(){  $("#submit-update").show(); }, 1000);
});
$("#submit-data2").on("click", function (e) {
    e.preventDefault();

    var o_data = $("#data_form2").find(':input').not('.no-serialize').serialize();
