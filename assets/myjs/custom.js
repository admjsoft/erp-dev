...
$("#addproduct").on("click", function () {
  ...
  var element = document.getElementById("system_level_tax");

  if (element !== null) {
    var system_tax = $("#system_level_tax").val();
    if (system_tax == 0) {
      var tax_status = "disabled";
    } else {
      var tax_status = "";
    }
  } else {
  ...
  //product row
  var data =
    '<tr><td><input type="text" class="form-control" name="product_name[]" placeholder="Enter Product name or Code" id="productname-' +
    cvalue +
    '"></td><td><input type="text" class="form-control req amnt" name="product_qty[]" id="amount-' +
    cvalue +
    '" onkeypress="return isNumber(event)" onkeyup="rowTotal(' +
    functionNum +
    '), billUpyog()" autocomplete="off" value="1"  inputmode="numeric"><input type="hidden" id="alert-' +
    cvalue +
    '" value=""  name="alert[]"> </td> <td><input type="text" class="form-control req prc" name="product_price[]" id="price-' +
    cvalue +
    '" onkeypress="return isNumber(event)" onkeyup="rowTotal(' +
    functionNum +
    '), billUpyog()" autocomplete="off" inputmode="numeric"></td><td> <input type="text" ' +
    tax_status +
    ' class="form-control vat" name="product_tax[]" id="vat-' +
    cvalue +
    '" onkeypress="return isNumber(event)" onkeyup="rowTotal(' +
    functionNum +
    '), billUpyog()" autocomplete="off" inputmode="numeric"></td> <td id="texttaxa-' +
    cvalue +
    '" class="text-center">0</td> <td><input type="text" class="form-control discount" name="product_discount[]" onkeypress="return isNumber(event)" id="discount-' +
    cvalue +
    '" onkeyup="rowTotal(' +
    functionNum +
    '), billUpyog()" autocomplete="off"></td> <td><span class="currenty">' +
    currency +
    "</span> <strong><span class='ttlText' id=\"result-" +
    cvalue +
    '">0</span></strong></td> <td class="text-center"><button type="button" data-rowid="' +
    cvalue +
    '" class="btn btn-danger removeProd" title="Remove" > <i class="fa fa-minus-square"></i> </button> </td><input type="hidden" name="taxa[]" id="taxa-' +
    cvalue +
    ...
    cvalue +
    '" value="0"> <input type="hidden" name="unit[]" id="unit-' +
    cvalue +
    '" value=""> <input type="hidden" name="hsn[]" id="hsn-' +
    cvalue +
    '" value=""> <input type="hidden" name="serial[]" id="serial-' +
    cvalue +
    '" value=""> </tr><tr><td colspan="8"><textarea class="form-control"  id="dpid-' +
    cvalue +
...
var rowTotal = function (numb) {
  ...
  if (tax_status == "yes") {
    ...
    if (disFormat == "%" || disFormat == "flat") {
      ...
      //alert(totalPrice);
      //alert(vatVal);
      var Inpercentage = (totalPrice * vatVal) / 100;
      //alert(Inpercentage);
  ...
  for (var i = 0; i < idArray.length; i++) {
    ...
    if (taxFormat == "%") {
      ...
      if (disFormat == "%" || disFormat == "flat") {
        ...
        if (disFormat == "%") {
          var total_tax = (result * vatVal) / 100;
          var total_with_tax = result + total_tax;
          var Inpercentage = precentCalc(total_with_tax, discountVal);
          result = total_with_tax - Inpercentage;
          $("#disca-" + x).val(accounting.formatNumber(Inpercentage));
          discsr = discsr + Inpercentage;
        } else if (disFormat == "flat") {
          ...
          var total_tax = (result * vatVal) / 100;
          var total_with_tax = result + total_tax;
          result = total_with_tax - discountVal;
          $("#disca-" + x).val(accounting.formatNumber(discountVal));
          discsr += discountVal;
        }
      } else {
        if (disFormat == "b_p") {
          ...
          result = result - Inpercentage;
          //alert(result);
          total_tax = (result * vatVal) / 100; 
          //alert(total_tax);
          result = result + total_tax;
          //alert(result);
          $("#disca-" + x).val(accounting.formatNumber(Inpercentage));
          discsr = discsr + Inpercentage;
        } else if (disFormat == "bflat") {
        ...
        // alert(vatVal);
        taxr = taxr + total_tax;
        $("#texttaxa-" + x).html(accounting.formatNumber(total_tax));
        $("#taxa-" + x).val(accounting.formatNumber(total_tax));
      }
    } else {
