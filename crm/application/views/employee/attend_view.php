...
<div id="c_body"></div>
 ...
 //$_SESSION["favanimal"] =$_SESSION['id'];
                        ...
                        if(isset($activeuser)) {
                            ...
                            echo ' <a class="nav-link nav-link-label" href="#" data-toggle="dropdown">'.$this->lang->line('Attendance').' <i class="ficon icon-clock"></i><span class="badge badge-pill badge-default badge-warning badge-default badge-up">' . $this->lang->line('Off') . '</span></a>';
                        }
                        ...
                        if (!$this->aauth->clock()) {
                            if(isset($activeuser)) {
                                    ...
                                    if($usr['id']==$emp)
                                    {
                                        echo '<a href="' . base_url() . '/dashboard/clock_out?id='.$usr['id'].'" class="btn btn-outline-danger  btn-outline-white btn-md col-xl-5 col-md-12 m-1" ><span class="icon-toggle-off" aria-hidden="true"></span> ' . $this->lang->line('ClockOut') . ' </a>';
                                            if($this->aauth->check_break()){
                                                ...
                                                echo '<a href="' . base_url() . 'dashboard/break_out" class="btn btn-outline-cyan btn-md col-xl-5 col-md-12 m-1" ><span class="icon-toggle-off" aria-hidden="true"></span> ' . $this->lang->line('End Break') . 'End Break</a>';
                                        }
                                    }
                                    else{
                                ...
                                echo '<a href="' . base_url() . '/dashboard/clock_in" class="btn btn-outline-success  btn-outline-white btn-md col-xl-5 col-md-12 m-1" ><span class="icon-toggle-on" aria-hidden="true"></span> ' . $this->lang->line('ClockIn') . ' <i class="ficon icon-clock spinner"></i></a>';
                            }

                        } else {

                            echo '<a href="' . base_url() . '/dashboard/clock_out" class="btn btn-outline-danger  btn-outline-white btn-md col-xl-5 col-md-12 m-1" ><span class="icon-toggle-off" aria-hidden="true"></span> ' . $this->lang->line('ClockOut') . ' </a>';
                                if($this->aauth->check_break()){
                                      $rw=$this->aauth->break_time_all();
                                        foreach($rw as $item){
                                        echo '<a href="' . base_url() . 'dashboard/break_in?bt='.$item['id'].'" class="btn btn-outline-light-blue  btn-outline-blue-grey  btn-md col-xl-5 col-md-12 col-xl-5 m-1" ><span class="icon-toggle-off" aria-hidden="true"></span> '.$item['name'].'</a>';
                                    }
                            }else{
                                    echo '<a href="' . base_url() . 'dashboard/break_out" class="btn btn-outline-cyan btn-md col-xl-5 col-md-12 m-1" ><span class="icon-toggle-off" aria-hidden="true"></span> ' . $this->lang->line('End Break') . 'End Break</a>';
                            }
