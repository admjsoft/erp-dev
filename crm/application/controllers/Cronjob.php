...
foreach($exppassportlist as $exppassport) {
    //do something
    $elements[] =  $exppassport['email'];
}
$passportexpiry= implode(',', $elements);
	
	$elements1 = array();
foreach($exppermitlist as $exppermit) {
    //do something
    $elements1[] =  $exppermit['email'];
}
$permitexpiry= implode(',', $elements1);
	
	 $mailto=$passportexpiry;
	 
         $mailtotitle="";
		 $subject="Passport Reminder";
		 $message="Hii Your Passport is going to expiry";
		 $attachmenttrue="true";
	    ...
	    $mailto1=$permitexpiry;
		$subject1="Permit Reminder";
		$message1="Hii Your Permit is going to expiry";
        $this->ultimatemailer->load($host, $port, $auth, $auth_type, $username, $password, $mailfrom, $mailfromtilte, $mailto1, $mailtotitle, $subject1, $message1, $attachmenttrue, $attachment);
