...
class Invoices_model extends CI_Model
{
    ...
    {
        $this->db->select('gtg_invoices.*,SUM(gtg_invoices.shipping + gtg_invoices.ship_tax) AS shipping,
		gtg_invoices.loc as loc,gtg_invoices.id AS iid,gtg_customers.id AS cid,gtg_customers.name,
		gtg_customers.phone,gtg_customers.address,gtg_customers.city,
		gtg_customers.region,gtg_customers.country,gtg_customers.postbox,gtg_customers.email,
		gtg_customers.company,gtg_customers.custom1,gtg_customers.name_s,
		gtg_customers.phone_s,gtg_customers.address_s,gtg_customers.city_s,
		gtg_customers.region_s,gtg_customers.country_s,gtg_customers.postbox_s,
		gtg_customers.email_s,gtg_customers.loc,gtg_customers.gid,gtg_terms.id AS termid,gtg_customers.taxid,
		gtg_terms.title AS termtit,gtg_terms.terms AS terms');
        $this->db->from($this->table);
    ...
    function count_peppol_invoices_filtered($opt = '')
    {
        $this->_get_peppol_invoices_datatables_query($opt);
        if ($opt) {
            $this->db->where('eid', $opt);
        }
        if ($this->aauth->get_user()->loc) {
    ...
    public function count_peppol_invoices_all($opt = '')
    {
        $this->db->select('gtg_peppol_invoices.id');
        $this->db->from($this->table);
        $this->db->join('gtg_peppol_invoices','gtg_invoices.id = gtg_peppol_invoices.invoice_id');
        $this->db->where('gtg_invoices.i_class', 0);
        if ($opt) {
            $this->db->where('gtg_invoices.eid', $opt);
        }
 ...
 private function _get_month_datatables_query()
  ...
  public function count_month_all($opt = '')
  {
    ...
    private function _get_peppol_invoices_datatables_query($opt = '')
    {
        ...
        if ($this->input->post('start_date') && $this->input->post('end_date')) // if datatable send POST for search
        {
            $this->db->where('DATE(gtg_peppol_invoices.invoice_sent_date) >=', datefordatabase($this->input->post('start_date')));
            $this->db->where('DATE(gtg_peppol_invoices.invoice_sent_date) <=', datefordatabase($this->input->post('end_date')));
        }
    ...
    function get_peppol_invoices_datatables($opt = '')
    {
        $this->_get_peppol_invoices_datatables_query($opt);
        if ($_POST['length'] != -1)
            $this->db->limit($_POST['length'], $_POST['start']);
