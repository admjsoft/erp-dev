...
class Quote_model extends CI_Model
{
    ...
    public function quote_delete($id)
    {
        $this->db->trans_start();
        if ($this->aauth->get_user()->loc) {
            $res = $this->db->delete('gtg_quotes', array('id' => $id, 'loc' => $this->aauth->get_user()->loc));
        } else {
            if (BDATA) {
                $res = $this->db->delete('gtg_quotes', array('id' => $id));
            } else {
    ...
    public function attach($id)
    {
        ...
        $this->db->where('gtg_metadata.type', 2);
