<div class="content-body">
    ...
    <div class="row mt-2">
    <div class="col-md-12">
        <h1 class="text-center">Invoice to Delivery Order Generation</h1>
...
<th scope="row">' . $c . '</th>
    ...
    foreach ($products as $row) {
        $sub_t += $row['price'] * $row['qty'];
        if(($row['total_delivery_qty'] - $row['return_qty']) < $row['qty'])
        {
...
<th scope="row">' . $c . '</th>
                            ...
                            <td>' . amountExchange($row['price'], 0, $this->aauth->get_user()->loc) . '</td>
                            <td>' . ((int)$row['qty']) . '</td>  
                            <td><input '.$do_items_d_status.' style="width: 50px;" invoice_id="'.$row['tid'].'" p_id="'.$row['pid'].'" class="xs form-control '.$do_items_class.'" type="number" value="' . ((int) ($row['qty'] - ($row['total_delivery_qty'] - $row['return_qty']))) . '" /></td>
                             <td>' . ((int) $row['total_delivery_qty'] - $row['return_qty']) . '</td>
                             <td>' . ((int) $row['return_qty']) . '</td>
                             <td>' . ($row['qty'] - ($row['total_delivery_qty'] - $row['return_qty'])) . '</td>
                        ...
                        <div class="col-lg-6 text-right">
                            ...
                            <div class="title-action">
                                ...
                                <a href="#" class="btn btn-success" onclick="getDataAndSendToController()">
                                    <i class="fa fa-briefcase"></i> <?php echo $this->lang->line('Generate DO') ?>
                                </a>
                                <?php } ?>
                            </div>
                        </div>
...
function getDataAndSendToController() {
    ...
    Swal.fire({
                title: "Do you want to Generate DO?",
    ...
    inputElements.forEach(function(input) {
        var inputAttributes = {
            ...
            value: input.value,
            invoice_id: input.getAttribute('invoice_id'),
    ...
    $.ajax({
        type: 'POST',
        dataType: 'json',
        url: '<?php echo base_url('deliveryorder/update_invoice_delivery_order') ?>',
        data: {
        ...
        success: function(response) {
            ...
            if (response.status == '200') {
                ...
                //location.reload();
                window.location.href = '<?php echo base_url('deliveryorder/list') ?>';
            } else {
                //alert(response.message);
                Swal.fire({
                      icon: "error",
