...
public function fwmseditcustomer()
{
	  ...
	  $this->load->library("Custom");
       ...
       // $data['payslip']=$this->payroll->getPayslipList();
          ...
          $data['client']=$this->customers->get_fws_client_list($id);
        $this->load->view('fixed/header', $head);
        $this->load->view('customers/fwmsclientedit', $data);
        $this->load->view('fixed/footer');	
...
public function getfwmsClients()
{
	
	    $ttype = $this->input->get('type');
        $list = $this->customers->get_datatables($ttype);
		...
		$type='';
	     foreach ($list as $prd) {
           ...
           // $row[] = amountExchange($prd->tax_amount, 0, $this->aauth->get_user()->loc);
            $row[] = '
			<a href="' . base_url().'customers/fwmseditcustomer/edit?id=' . $pid . '" class="btn btn-info btn-sm"  title="Edit">
			<span class="fa fa-edit"></span></a>&nbsp;<a  href="#" data-object-id="' . $pid . '" class="btn btn-danger btn-sm delete-object"><span class="fa fa-trash"></span></a>';
            //$row[] =$temp;
            /*
...
public function saveInternational()
{
	...
	if(!$insert){
                    ...
                    $data['status'] = 'success';
                    $data['message'] = $this->lang->line('Client Created Successfully');
}
...
public function updateInternational()
{
	...
	if(!$update){
                    ...
                    $data['status'] = 'success';
                    $data['message'] = $this->lang->line('Client Updated Successfully');
}
	     $_SESSION['status']=$data['status'];
        $_SESSION['message']=$data['message'];
        $this->session->mark_as_flash('status');
...
public function deleteFwmsClient()

{
    ...
    public function balance()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($this->customers->rechargewalletbyadmin($id, $amount)) {
