...
class Settings extends CI_Controller
{
    ...
    public function company()
    {
        ...
        if ($this->input->post()) {
            ...
            $android_app_link = $this->input->post('android_app_link', true);
            $version = $this->input->post('version', true);
            $this->settings->update_company(1, $name, $phone, $email, $address, $city, $region, $country, $postbox, $taxid, $data_share, $foundation, $android_app_link, $version);
        } else {
    ...
    public function delete_custom_field()
    {
        ...
        if ($this->db->delete('gtg_custom_fields', array('id' => $id))) {
            $this->db->delete('gtg_custom_data', array('field_id' => $id));
            echo json_encode(array('status' => 'Success', 'message' => $this->lang->line('DELETED')));
        } else {
            echo json_encode(array('status' => 'Error', 'message' => $this->lang->line('ERROR')));
        }
...
FROM gtg_invoices AS i LEFT JOIN gtg_customers AS c ON i.csd=c.id GROUP BY  i.csd ORDER BY  i.id  LIMIT 10");
        $error = $this->db->error();
        if (@$error['code']) {
