...
//die;
 ...
 public function subscribe($module_id)
    ...
    {
        $this->aauth_db->select('gtg_pms.*,gtg_pms.id AS pid,gtg_employees.*');
        $this->aauth_db->from('gtg_pms');
        if (is_numeric($receiver_id)) {
            $query = $this->aauth_db->where('gtg_pms.receiver_id', $receiver_id);
            $query = $this->aauth_db->where('gtg_pms.pm_deleted_receiver', 0);
        }
        if (is_numeric($sender_id)) {
            $query = $this->aauth_db->where('gtg_pms.sender_id', $sender_id);
            $query = $this->aauth_db->where('gtg_pms.pm_deleted_sender', 0);
        }

        $this->aauth_db->order_by('gtg_pms.id', 'DESC');
        $this->aauth_db->join('gtg_employees', 'gtg_employees.id = gtg_pms.sender_id', 'left');
        $this->aauth_db->limit($limit, $offset);
    ...
    {
        $this->aauth_db->insert('gtg_log', array('note' => $input1, 'user' => $input2, 'created' => date('Y-m-d H:i:s')));
    }

    public function clock($c_user_id = '')
    {

        $this->aauth_db->select('clock');
        if(!empty($c_user_id))
        {
            $this->aauth_db->where('id', $c_user_id);
        }else{
            $this->aauth_db->where('id', $this->CI->session->userdata('id'));
        }
        
        $this->aauth_db->from('gtg_employees');
        $query = $this->aauth_db->get();
        $emp = $query->row_array();
        if (!is_array($emp)){
        return null;
    ...
    public function check_break($c_user_id = '')
    {
        ...
        $today = date('Y-m-d');
        $this->aauth_db->select('status');
        $this->aauth_db->where('emp', $id);
        $this->aauth_db->where('bdate', $today);
        $this->aauth_db->order_by("clockin","desc");
        $this->aauth_db->limit(1);
        $this->aauth_db->from('gtg_attend_break');
        $query = $this->aauth_db->get();
        $emp = $query->row_array();
        $temp=true;
    ...
    function break_time_all()
    {
        $this->aauth_db->select('*');
        $this->aauth_db->from('gtg_break_time');
        $query = $this->aauth_db->get();
        return $query->result_array();
