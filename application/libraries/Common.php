...
class Common
{
    ...
    function languages($id = 0)
    {
        if ($id) {
            ...
            $this->PI->db->from('gtg_locations');
            $this->PI->db->where('id', $id);
            $query = $this->PI->db->get();
            $out = $query->row_array();
        } else {
            $this->PI->db->select('*');
            $this->PI->db->from('gtg_system');
    ...
    public function default_warehouse()
    {
        if ($this->PI->aauth->get_user()->loc) {
            ...
            $this->PI->db->select('gtg_locations.ware,gtg_warehouse.title');
            $this->PI->db->from('gtg_locations');
            $this->PI->db->join('gtg_warehouse', 'gtg_locations.ware=gtg_warehouse.id', 'left');
            ...
            $wr = '<option value="0">*' . $this->PI->lang->line('All') . '</option>';
            $this->PI->db->select('univarsal_api.key1,gtg_warehouse.title');
            $this->PI->db->from('univarsal_api');
            $this->PI->db->join('gtg_warehouse', 'univarsal_api.key1=gtg_warehouse.id', 'left');
